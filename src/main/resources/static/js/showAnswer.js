...
$(document).ready(function() {
	...
	let answerId = getQueryString("answerId");
	if (answerId == null) {
		window.open("answer.html", "_self");
	} else if ((parseInt(answerId) != answerId) || (answerId < 0)) {
		jump404();
	}

	function loadAnswerAndAuthor() {
		//获取回答和回答者信息并填充DOM
		new Promise(function(resolve, reject) {
			$.ajax({
				...
				success: function(response) {
					...
					if (response != null && response != "") {
						...
						if (response.headPhotoUrl != null && response.headPhotoUrl != "") {
							$(".mobile_answererHeadPhoto img").attr("src", response.headPhotoUrl);
						}
						$(".answererName").html(response.nickname);
						$(".mottoWord").html(response.motto);
						//填充PC端右侧回答者信息DOM
						if (response.headPhotoUrl != null && response.headPhotoUrl != "") {
							$(".answererBasicInfo img").attr("src", response.headPhotoUrl);
						}
						$(".answerCount").html(response.answerCount);
						$(".articleCount").html(response.articleCount);
						$(".fansCount").html(response.followerCount);
						//填充回答主体
						$(".answerTitle h1").html(response.answer.title);
						let submitTime = new Date(response.answer.answer_time);
						...
						//若回答有修改，则填充修改时间并显示
						if (response.answer.last_update_time != null && response.answer.last_update_time != "") {
							let lastUpdateTime = new Date(response.answer.last_update_time);
						...
						$(".browseCountValue").html("被浏览次数：" + response.answer.browse_count);
						//填充回答内容Markdown
						$("#mainContent textarea").html(response.answer.content);
						//转换正文html
						...
						$(".mobile_answererHeadPhoto,.answererName,.answererNameAndMotto .answererName,.goToMainPageOfTA,.answererHeadPhoto").on("click",function() {
							window.open("user.html?visitUid=" + response.answer.answerer,"_self");
						});
						//绑定问题标题和回答个数点击事件
						$(".clickToQuestion").attr("href","showQuestion.html?questionId=" + response.questionId);
						//修改回答个数
						$(".questionHowManyAnswerNum").html(response.questionAnswerCount);
						//修改问题标题
						$(".questionTitleWord > a > h1").html(response.questionTitle);
						//取消占位符显示
						$(".loadingHolder").css("display", "none");
						//处理从属关系
						refreshRelation(response.answer.answerer,
	...
	function refreshRelation(authorId, isAlreadyFollow, isAlreadyCollect) {
		...
		//回答者是本人
			...
			$(".answererHeadPhoto,.mobile_answererHeadPhoto").on("click", function() {
				window.open("me.html", "_self");
			});
		}
		//回答者不是本人
	...
	function checkReachContentBottom() {
		if (checkVisible($("#checkReachBottom")[0]) || !checkVisible($("#mainContent")[0])) {
			...
			$(".fixedActions").css("width", $(".answerTop").outerWidth());
	...
	$(".commentSubmit").on("click", function() {
		if (loginUser != null && loginUser != "") {
			$.ajax({
				...
				success: function(response) {
					if (response.success == "true" && response.type == "answer") {
						$("#success_modal")
